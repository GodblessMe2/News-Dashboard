...
module.exports = __webpack_require__("./src/assets/js/main.js");
...
const bodyInput = document.getElementById('mytextarea');

// console.log(image);

loadAllEventListener();
function loadAllEventListener() {
  // chooseFile.addEventListener('change', getImageData);
  blogPost.addEventListener('submit', addPost);
}
...
function addPost(e) {
  ...
  const titlePage = titleInput.value;
  const bodyPost = bodyInput.value;

 if (titlePage === '' && bodyPost === '') {
   alert('Please Add The Following Details');
 };
